  ...
  return (
    <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "80px 24px 96px" }}>
      <style>{`
        ...
        @media (max-width: 768px) { .sobre-mi-grid { grid-template-columns: 1fr !important; gap: 40px !important; } }
      `}</style>

      <div style={{ marginBottom: "64px" }}>
        <div className="label-tag" style={{ marginBottom: "16px" }}>SOBRE MÍ</div>
        <h1 style={{ fontSize: "clamp(40px, 5vw, 56px)", fontWeight: 800, letterSpacing: "-0.02em", lineHeight: 1.1, color: "var(--text)", marginBottom: "10px" }}>
          José G. Helú
        </h1>
      ...
      <div className="sobre-mi-grid" style={{ display: "grid", gridTemplateColumns: "1fr 360px", gap: "64px", alignItems: "start" }}>
        <div>
          <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
            {[
              ...
              <p key={i} style={{ fontSize: "17px", color: i === 0 ? "var(--text)" : "var(--muted)", lineHeight: 1.75, fontWeight: i === 0 ? 500 : 400 }}>
                {text}
              </p>
            ))}
          </div>
          <div style={{ marginTop: "48px" }}>
            <Link href="/agenda" className="sobre-cta">Agenda una consulta →</Link>
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          <div style={{ width: "100%", maxWidth: "280px", aspectRatio: "1", borderRadius: "16px", background: "var(--card)", border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: "10px" }}>
            ...
            <p style={{ fontSize: "12px", color: "var(--muted)", textAlign: "center" }}>Foto próximamente</p>
          </div>

          <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "12px", padding: "22px 20px" }}>
            <div className="label-tag" style={{ marginBottom: "16px" }}>CREDENCIALES</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
              {credentials.map(c => (
                <span key={c} style={{ background: "var(--accent-gold-muted)", border: "1px solid var(--border-gold)", borderRadius: "6px", padding: "4px 12px", fontSize: "12px", fontWeight: 600, color: "var(--accent-gold)" }}>
                  {c}
                </span>
              ))}
            </div>
          ...
          <div style={{ background: "var(--card)", border: "1px solid var(--border-blue)", borderRadius: "12px", padding: "20px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              <a href="mailto:jose@dedaloproteccion.mx" style={{ fontSize: "14px", color: "var(--accent-gold)", textDecoration: "none" }}>
                jose@dedaloproteccion.mx
              </a>
              <a href="https://wa.me/526861085164" target="_blank" rel="noopener noreferrer" style={{ fontSize: "14px", color: "var(--muted)", textDecoration: "none" }}>
